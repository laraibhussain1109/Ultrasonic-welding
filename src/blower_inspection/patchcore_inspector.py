...
class PatchCoreInspector(HybridPatchcorePadimInspector):
    ...
    def train(self, config: PartModelConfig, progress_callback=None) -> Path:
        ...
        for i, path in enumerate(paths, 1):
            ...
            try:
                roi = self._canonical(frame, config, detector) if frame is not None else None
                result = quality.analyze(roi) if roi is not None else None
            except ValueError:
                result = None
            if result is None or not result.valid:
                reasons = result.reasons if result else ("BAD_CROP",)
                for reason in reasons: rejection_counts[reason] = rejection_counts.get(reason, 0) + 1
            else:
                accepted.append(roi); accepted_paths.append(path)
        ...
        if len(accepted) < 10:
            raise ValueError(f"Only {len(accepted)} diverse, qualified images remain; at least 10 are required")
        ...
        report = {"total_source_images": len(paths), "accepted_images": len(accepted),
                  ...
                  "duplicates_removed": len(keep) - len(accepted), "rejected": rejection_counts}
        model_path.with_suffix(".training_report.json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
