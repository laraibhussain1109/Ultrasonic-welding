"""Fail-closed NVIDIA TAO Deploy visual-anomaly runtime.
...
class TaoInspector:
    """Run a TAO-exported ONNX anomaly model with fail-closed safeguards."""
    ...
    def _session(self, config: PartModelConfig) -> Any:
        ...
        if len(session.get_inputs()) != 1:
            raise RuntimeError("TAO model must expose exactly one image input")
    ...
    def _infer(self, config: PartModelConfig, image: np.ndarray) -> tuple[np.ndarray, float]:
        session = self._session(config)
        input_meta = session.get_inputs()[0]
        input_name = config.tao_input_name or input_meta.name
        outputs = session.run(None, {input_name: self._input_tensor(image, input_meta.shape)})
        ...
        anomaly_map = np.asarray(values[map_name], dtype=np.float32).squeeze()
    ...
    def train(self, config: PartModelConfig) -> Path:
        ...
        for path in paths:
            ...
            crop = crop_component_roi(image, roi_ratios=config.roi_ratios)
            anomaly_map, score = self._infer(config, crop)
        ...
        calibration = TaoCalibration(_sha256(config.model_file), pixel_threshold, image_threshold, p999, score_p999, len(paths), datetime.now(timezone.utc).isoformat())
    ...
    def inspect(self, config: PartModelConfig, image: np.ndarray, *, save_outputs: bool = True, crop_to_component: bool = True) -> InspectionResult:
        ...
        raw_map, image_score = self._infer(config, image)
