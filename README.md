...
By default the live camera stream is opened at 1920×1080 @ 30 FPS instead of the full 8.3 MP sensor frame. The blower fan ROI is still cropped before inference, but reducing USB/camera bandwidth avoids the 1 FPS full-frame capture bottleneck. Override these capture settings only if the camera/nest requires it:

```powershell
$env:BLOWER_INSPECTION_CAMERA_WIDTH = "1920"
$env:BLOWER_INSPECTION_CAMERA_HEIGHT = "1080"
$env:BLOWER_INSPECTION_CAMERA_FPS = "30"
python -m blower_inspection.app
```
